...
const widgetNotYetLoadedWarning = () => {
  console.warn(
    `loadImmediately is set to false and widget.load() has not been called on window.${_constants.GLOBAL_VARIABLE} yet. Please call widget.load() first or set loadImmediately on window.${_constants.SETTINGS_VARIABLE} to true.`
  );
};
...
function createWidgetShell({ eventEmitter, logError = noop }) {
  const embedScriptContext = (0,
  _createEmbedScriptContext.createEmbedScriptContext)();
  const widgetShell = new _WidgetShell.WidgetShell(
    embedScriptContext,
    eventEmitter,
    logError
  );
