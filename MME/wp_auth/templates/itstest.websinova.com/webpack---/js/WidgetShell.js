...
var _PageTitleNotificationsPlugin = _interopRequireDefault(
  require("./page-title-notifications/PageTitleNotificationsPlugin")
);
...
var _ScrollPercentageTracker = _interopRequireDefault(
  require("./scroll-percentage/ScrollPercentageTracker")
);

var _ExitIntentTracker = _interopRequireDefault(
  require("./exit-intent/ExitIntentTracker")
);
...
class WidgetShell {
  constructor(embedScriptContext, eventEmitter, errorLogger) {
    this.loadIFrame = () => {
      ...
      if ((0, _settingsHelpers.shouldEmbedInline)()) {
        const embedElement = document.querySelector(
          (0, _settingsHelpers.getInlineEmbedSelector)()
        );

        if (!embedElement) {
          this.devLogger.error(
            `cannot embed widget - element at \`${(0,
            _settingsHelpers.getInlineEmbedSelector)()}\` cannot be found`
          );
          return;
        }
      ...
      if (!document.getElementById(_elementSelectors.PARENT_ID)) {
        ...
        const embeddedInProduct = (0, _isEmbeddedInProduct.isEmbeddedInProduct)(
    ...
    this.handleI18nLabels = ({ data }) => {
      if (!this.iframe || !data) return;
      const frameLabel =
        data["conversations-visitor-ui.visitorExperienceAriaLabels.chatWidget"];
    ...
    this.requestWidgetRefresh = (0, _throttle.throttle)(
    ...
    const postMessageApiClient = new _PostMessageApiClient.PostMessageApiClient(
    ...
    this.postMessageReceiver = new _PostMessageReceiver.PostMessageReceiver(
      {
        [_pageTitleNotificationsPostMessageTypes.SHOW_PAGE_TITLE_NOTIFICATION]:
          pageTitleNotifications.handleShow,
        [_pageTitleNotificationsPostMessageTypes.CLEAR_PAGE_TITLE_NOTIFICATION]:
          pageTitleNotifications.handleClear,
        ...
        [_receivedPostMessageTypes.CLOSED_WELCOME_MESSAGE]:
          _hideWelcomeMessage.hideWelcomeMessage,
        [_receivedPostMessageTypes.STORE_MESSAGES_COOKIE]:
          this.handleStoreMessagesCookie,
        [_receivedPostMessageTypes.EXTERNAL_API_EVENT]:
          this.handleExternalApiEventMessage,
        [_receivedPostMessageTypes.API_REQUEST]:
          postMessageApiClient.makeApiRequest,
  ...
  removeIframe() {
    const iframeContainer = (0, _settingsHelpers.shouldEmbedInline)()
      ? document.getElementById(_elementSelectors.INLINE_PARENT_ID)
      : document.getElementById(_elementSelectors.PARENT_ID);
  ...
  handleResizeMessage({ data: { height, width } = {} }) {
  ...
  setWidgetOpenCookie({ isOpen }) {
    (0, _operators.setCookie)(
      _constants.cookies.IS_OPEN,
      isOpen,
      _times.default.THIRTY_MINUTES
    );
  }

  handleOpenChange({ data: { isOpen, isUser } }) {
    const html = document.documentElement;
    const parent = document.getElementById(_elementSelectors.PARENT_ID);
    const shadowContainer = parent.getElementsByClassName(
      _widgetClassNames.SHADOW_CONTAINER
    )[0];
    this.isOpen = isOpen;
  ...
  handleStoreMessagesCookie({ data }) {
    this.iframeMessage.post(_sentPostMessageTypes.FIRST_VISITOR_SESSION, {
      isFirstVisitorSession: false,
    });

    if (
      (0, _getGlobalCookieOptOut.getGlobalCookieOptOut)() === "yes" &&
      this.widgetData.gdprConsentOptions.cookieConsentPrompt !==
        _gdprCookieConsentTypes.NEVER
    ) {
  ...
  requestWidgetRefresh({ openToNewThread } = {}) {
    const { portalId } = this.embedScriptContext;
  ...
  handleWidgetRefresh(refreshedWidgetData) {
    ...
    const shouldHideWidget =
      (0, _getWidgetDataResponseType.getWidgetDataResponseType)(
        this.widgetData
      ) === _widgetResponseTypes.HIDE_WIDGET;

    if (shouldHideWidget) {
      this.removeIframe();
    } else {
      this.iframeMessage.post(
        _sentPostMessageTypes.REFRESH_WIDGET_DATA,
        Object.assign(
          {},
          this.widgetData,
          {},
          (0, _getIframeQueryParams.getIframeQueryParams)(
            this.embedScriptContext
          )
        )
      );
  ...
  loadWidget(options = {}) {
    ...
    (0, _fetchWidgetData.fetchWidgetData)(
      ...
      (0, _handleTargetingAndDelay.handleTargetingAndDelay)(
        this.setWidgetData,
        this.loadIFrame,
  ...
  start() {
    const { shouldRender } = (0, _shouldRenderWidget.shouldRenderWidget)(
      this.embedScriptContext
    );
