...
var _EmbedScriptContext = _interopRequireDefault(
  require("./EmbedScriptContext")
);

var _settingsHelpers = require("../external-api/settingsHelpers");

function createEmbedScriptContext() {
  const { messagesEnv, portalId, messagesHublet } = (0,
  _envGetters.getScriptEnvParams)();
  let useLocalBuild = false;

  try {
    useLocalBuild =
      localStorage && localStorage["live-chat-local-toggle"] === "true"; // eslint-disable-next-line no-empty
  } catch (e) {}

  const iFrameDomainOverride =
    typeof messagesConfig !== "undefined" && messagesConfig.iFrameDomain;
