...
function getInternalRequestUrl({
  ...
  const usersPortalId = (0, _getPortalIdFromPath.getPortalIdFromPath)(
    window.location.pathname
  );
  return `${(0, _getApiDomain.getApiDomain)(
    messagesEnv,
    messagesHublet
  )}/livechat/v1/message/public/hubspot-app?portalId=${usersPortalId}&mobile=${(0,
  _whichDevice.isAnyMobile)()}&embeddedPortalId=${portalId}&traceId=${messagesUtk}`;
}
...
function buildRequestParams({
  ...
  let requestUrl = `?portalId=${portalId}&${
    _legacyHubspotBenderContext.bender.project
  }=${
    _legacyHubspotBenderContext.bender.depVersions[
      _legacyHubspotBenderContext.bender.project
    ]
  }&mobile=${(0, _whichDevice.isAnyMobile)()}`;
