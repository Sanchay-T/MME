...
const registerCookieListeners = ({ postMessageToIframe }) => {
  const handlePrivacyConsent = (consent) => {
    const allowedMessagesUtkCookie = consent.categories
      ? consent.categories.functionality
      : consent.allowed;
    const globalCookieOptOut = allowedMessagesUtkCookie
      ? _constants.cookieValues.GLOBAL_COOKIE_OPT_OUT_NO
      : _constants.cookieValues.GLOBAL_COOKIE_OPT_OUT_YES;
  ...
  window._hsq.push([
    ...
    (utk) =>
      postMessageToIframe(_sentPostMessageTypes.HUBSPOT_UTK, {
        utk,
      }),
  ]);
};
