...
const getDelayLoadingWidgetIframe = (widgetData, mobile) => {
  ...
  const { displayOnTimeDelay } = clientTriggers;
  const { enabled, timeDelaySeconds } = displayOnTimeDelay;
  ...
  return {
    shouldDelayLoadingIframe:
      !popOpenWidget && !popOpenWelcomeMessage && enabled,
    timeDelay,
  };
};
