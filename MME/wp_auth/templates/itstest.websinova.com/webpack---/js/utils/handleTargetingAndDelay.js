...
const handleTargetingAndDelay = (
  ...
  return (widgetData) => {
    const hideWidget =
      (0, _getWidgetDataResponseType.getWidgetDataResponseType)(widgetData) ===
      _widgetResponseTypes.HIDE_WIDGET;

    const initialize = !hideWidget && !!widgetData.sessionId;

    if (initialize) {
      const { shouldDelayLoadingIframe, timeDelay } = (0,
      _getDelayLoadingWidgetIframe.getDelayLoadingWidgetIframe)(
        widgetData,
        (0, _whichDevice.isAnyMobile)()
      );
