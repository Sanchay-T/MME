...
function isEmbeddedInProduct({
  ...
  const isPublicPricingPage =
    pathname.indexOf(PRICING_SUBSTRING) !== -1 &&
    !IN_APP_PRICING_PAGE_PATTERN.test(pathname);
  ...
  if (isInAppPage && !isPublicPricingPage && !isSignupPage) {
    if (hostname.indexOf("qa") !== -1) {
      return (
        portalId === PORTAL_53 ||
        portalId === BET_PORTAL_53 ||
        portalId === QA_PRICING_PAGE_PORTAL
      );
    }
