(function ($) {
  ...
  $(document).ready(function () {
    ...
    popularSearch.on("click", function (e) {
      ...
      $(this)
        .parent(".betterdocs-popular-search-keyword")
        .siblings(".betterdocs-searchform")
        .find(".betterdocs-search-field")
        .val(popularKeyword)
    ...
    searchField.on("input propertychange paste", function (e) {
      searchField.each(function () {
        ...
        let inputCat = thisEvent
          .parent(".betterdocs-searchform-input-wrap")
          .siblings(".betterdocs-search-category")
          .find(":selected")
          .val();
        let resultWrapper = thisEvent.parent().parent(".betterdocs-searchform");
        let kbSlug = thisEvent
          .parent()
          .parent(".betterdocs-searchform")
          .find(".betterdocs-search-kbslug")
          .val();

        //Before the result is fetched, we have to remove the search result wrapper based on letter limit characters
        if (!inputVal.length) {
          var nodeResults = thisEvent
            .parents(".betterdocs-live-search")
            .find(".betterdocs-search-result-wrap");
    ...
    $(".betterdocs-searchform .betterdocs-search-category").on(
      "change",
      function (e) {
        let thisEvent = $(this);
        let inputVal = thisEvent
          .siblings(".betterdocs-searchform-input-wrap")
          .children(".betterdocs-search-field")
          .val();
    ...
    function liveSearchAction(
      ...
      if (
        ...
        thisEvent
          .parent()
          .parent(".betterdocs-live-search")
          .find(".betterdocs-search-result-wrap")
          .addClass("hideArrow");
        thisEvent
          .parent()
          .parent(".betterdocs-live-search")
          .find(".docs-search-result")
          .slideUp(300);
        searchLoader.hide();
        searchClose.hide();
      }
      ...
      request = $.ajax({
        ...
        success: function (response) {
          ...
          if (inputVal2.length < search_letter_limit) {
            var nodeResults = $(".betterdocs-live-search").find(
    ...
    $("body").on("click", ".betterdocs-print-btn", function (event) {
      ...
      if ($("#betterdocs-entry-title").length) {
        entryTitle = document.getElementById(
      ...
      var pwidth = document.getElementById(
        "betterdocs-single-content"
      ).offsetWidth;
    ...
    if (active_subcategory.length) {
      ...
      while (
        subcat.attr("class") === "docs-sub-cat" ||
        subcat.attr("class") === "docs-sub-cat current-sub-cat"
      ) {
    ...
    catHeading.click(function (e) {
      ...
      if ($this.next(catList).hasClass("show")) {
        $this.next(catList).slideUp().removeClass("show");
      } else if (catList.hasClass("show")) {
        catList.slideUp().removeClass("show");
        $this.next(catList).slideToggle().toggleClass("show");
      } else {
        $this.next(catList).slideToggle().toggleClass("show");
      }
    });
    ...
    docSubCat.each(function () {
      $(this).click(function (e) {
        ...
        $(this).next(".docs-sub-cat, .docs-sub-cat-list").slideToggle();
      });
    });
    ...
    $(".betterdocs-faq-post-layout-2").on("click", function (e) {
      ...
      if (!current_node.parent().hasClass("active")) {
        current_node.parent().addClass("active");
        current_node
          .children(".betterdocs-faq-post-layout-2-icon-group")
          .children("svg")
          .toggle();
        current_node.next().slideDown();
      } else {
      ...
      if (
        ...
        jQuery.ajax({
          ...
          success: function (res) {
            if (res === true) {
              $(
                ".betterdocs-article-reactions-heading,.betterdocs-article-reaction-links"
              ).fadeOut(1000);
              $(".betterdocs-article-reactions")
                .html("<p>" + betterdocspublic.FEEDBACK.SUCCESS + "</p>")
                .fadeIn(1000);
            }
          },
        });
